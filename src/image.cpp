...
static struct hsv to_hsv(const stbi_uc r, const stbi_uc g, const stbi_uc b) {
...
void preprocess_image(struct img& image, const struct cfg& config) {
	...
	stbi_uc* idata = image.data;
	std::map<uint32_t, size_t> counts;
	stbi_uc r = 0, g = 0, b = 0;
	size_t max = 0;
	for (size_t i = 0; i < size; i += 3) {
		const size_t& count = ++counts[(idata[i + 2] << 16) | (idata[i + 1] << 8) | idata[i]];
		if (count > max) {
			max = count;
			r = idata[i + 0];
			g = idata[i + 1];
	...
	for (size_t i = 0; i < size; i += 3) {
		const struct hsv pixel = to_hsv(idata[i], idata[i + 1], idata[i + 2]);
		if ((std::abs(pixel.v - background.v) < config.thr_diff_v && std::abs(pixel.s - background.s) < config.thr_diff_s
			&& !(std::abs(pixel.h - background.h) >= config.thr_diff_h
				&& pixel.v >= config.thr_h_v && pixel.s >= config.thr_h_s))
			|| (config.more_bg && (config.dark ? pixel.v < background.v + config.thr_more_v : pixel.v > background.v - config.thr_more_v) 
				&& (std::abs(pixel.s - background.s) < config.thr_diff_s || std::abs(pixel.h - background.h) < config.thr_diff_h))) {
			idata[i + 0] = r;
			idata[i + 1] = g;
			idata[i + 2] = b;
		}
...
static inline void res_sample(struct rgb** sample, size_t n, struct rgb* const* colours, size_t size) {
...
static constexpr double sq_dist(const struct rgb& left, const struct rgb& right) {
	return sq_dist(left.r, left.g, left.b, right.r, right.g, right.b);
}

// k-means++, based on Arthur and Vassilvitskii (2007)
static inline void k_means_pp(struct img& image, struct rgb** sample, size_t n) {
...
// k-means clustering
static inline void k_means(struct img& image, struct rgb** sample, size_t n, const struct cfg& config) {
	unsigned char* means = new unsigned char[n]();
	bool changed = true;
	for (int iterations = 0; (iterations < config.iterations || config.iterations == 0) && changed; iterations++) {
		changed = false;
...
void process_image(struct img& image, const struct cfg& config) {
	...
	for (size_t i = 0; i < size; i += 3) {
		struct rgb* colour = reinterpret_cast<struct rgb*>(&image.data[i]);
	...
	struct rgb** sample = new struct rgb*[n];
